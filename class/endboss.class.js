class Endboss extends MoveableObjects {
  ...
  followSharkieY() {
    ...
    let step = diff * this.yFollowFactor; // eased approach -> reacts with a slight delay
    step = Math.max(-this.speedY, Math.min(this.speedY, step)); // capped -> slower vertical speed
    this.y += step;
  }
